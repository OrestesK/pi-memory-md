...
find_settings() {
	...
	if [ -f "$global_settings" ] && grep -q '"pi-memory-md"' "$global_settings" 2>/dev/null; then
...
main() {
	...
	LOCAL_PATH=$(expand_home_path "$LOCAL_PATH")
	...
	if [ -z "$REPO_URL" ]; then
		...
		git -C "$LOCAL_PATH" pull origin main 2>/dev/null || git -C "$LOCAL_PATH" pull origin master 2>/dev/null || log "No remote changes"
	fi

	# 4. Check if project memory is already initialized
	if [ -d "$PROJECT_DIR/core/project" ]; then
